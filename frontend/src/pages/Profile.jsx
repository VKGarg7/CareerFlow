...
function Field({ label, value }) {
  return (
    <div className="min-w-0">
      ...
      <p className="text-sm text-gray-800 truncate" title={value || undefined}>{value || <span className="text-gray-400 italic">—</span>}</p>
    </div>
  )
}

function Input({ label, value, onChange, type = 'text', placeholder = '', textarea = false, required = false }) {
  const cls = "w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white hover:border-gray-300 transition"
  return (
    <div>
...
export default function Profile() {
  ...
  return (
    <Layout>
      <PageHeader
        title="My Profile"
        subtitle="Manage your personal details, experience, and documents"
        icon="👤"
        gradient="from-blue-500 to-indigo-600"
        action={
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/change-password')}
              className="px-4 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 active:scale-95 transition">
              Change Password
            </button>
            ...
            {!editing && (
              <button onClick={() => { setEditing(true); setError(''); setSuccess('') }}
                className="px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-br from-blue-600 to-indigo-600 rounded-xl hover:shadow-lg hover:shadow-blue-200 hover:-translate-y-0.5 active:scale-95 transition-all shadow-sm">
      ...
      {success  && <Alert severity="success" onClose={() => setSuccess('')} sx={{ mb: 3, borderRadius: 2 }} className="animate-fade-slide-up">{success}</Alert>}
      {error    && <Alert severity="error"   onClose={() => setError('')}   sx={{ mb: 3, borderRadius: 2 }} className="animate-fade-slide-up">{error}</Alert>}
      {docError && <Alert severity="error"   onClose={() => setDocError('')} sx={{ mb: 3, borderRadius: 2 }} className="animate-fade-slide-up">{docError}</Alert>}
        ...
        <form key="edit" onSubmit={handleSubmit} className="space-y-8 mt-6 animate-fade-slide-up">
          ...
          <Section title="Education" onAdd={addItem('education', emptyEducation)}>
            {form.education.map((edu, i) => (
              <div key={i} className="border border-gray-100 rounded-2xl p-4 space-y-3 relative bg-gray-50/50 animate-scale-in">
          ...
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={saving}
              className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white text-sm font-semibold rounded-xl hover:bg-blue-700 active:scale-95 disabled:opacity-50 disabled:active:scale-100 transition shadow-sm">
              {saving && <CircularProgress size={14} color="inherit" />}
              {saving ? 'Saving...' : 'Save Profile'}
            </button>
            <button type="button"
              onClick={() => { setEditing(false); setError('') }}
              className="px-6 py-2.5 bg-gray-100 text-gray-700 text-sm font-semibold rounded-xl hover:bg-gray-200 active:scale-95 transition">
              Cancel
            </button>
          </div>
        </form>
...
function DocumentsSection({
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6 hover:shadow-md transition-shadow duration-200">
      <div className="flex items-center gap-2.5 mb-4 pb-2 border-b border-gray-100">
        <span className="w-7 h-7 rounded-lg bg-blue-50 flex items-center justify-center text-sm shrink-0">📄</span>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          {resumes.length === 0 ? (
            ...
            <div className="space-y-2">
              {resumes.map((r) => (
                <div key={r.id} className="flex flex-wrap items-center gap-3 p-3 rounded-xl border border-gray-200 bg-gray-50">
        ...
        <div>
          ...
          {coverLetter ? (
            <div className="flex flex-wrap items-center gap-3 p-3 rounded-xl border border-gray-200 bg-gray-50">
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest border-b border-gray-100 pb-2 w-full">{title}</h3>
        {onAdd && (
          <button type="button" onClick={onAdd}
            className="ml-4 whitespace-nowrap text-xs font-bold text-blue-600 hover:text-blue-700 active:scale-95 transition-transform">
            + Add
          </button>
        )}
      </div>
...
function Card({ title, icon, delay = 0, children }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 hover:shadow-md transition-shadow duration-200 animate-fade-slide-up"
      style={{ animationDelay: `${delay * 60}ms`, animationFillMode: 'backwards' }}>
      <div className="flex items-center gap-2.5 mb-4 pb-2 border-b border-gray-100">
        {icon && (
          <span className="w-7 h-7 rounded-lg bg-blue-50 flex items-center justify-center text-sm shrink-0">{icon}</span>
        )}
